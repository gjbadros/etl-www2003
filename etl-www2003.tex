...
%%% 
%% GREGB:FIXME:: avoid too obvious of an overfull hbox below
...
to the PHP example from \figref{php-books}.  The file is an XML
document with root element \smtexttt{bl:template}\footnote{We use
...
either output text to the current destination stream or perform some
side-effect (or both). For example the \smtexttt{bl:set} primitive
assigns a variable a value without outputting any text, while the
