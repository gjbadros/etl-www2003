...
obviously important property that a template execution terminates is
...
mechanism and reduce scalability).  Languages such as actually PHP
...
schema-aware XML editors to easily provide syntax colorization and
...
Server.  ETLS is currently employed by InfoSpace to serve tens of
...
Finally, \secref{related-work} describes and contrasts related work
...
enable automatic translation of our legacy templating language into
ETL\@. That need occasionally complicated our design. This paper
...
XML Schemas~\cite{XMLSchema1,XMLSchema2}.  Those schemata are used by
our development environment (see \figref{homesite-screenshot}) to support syntax
colorization and completion of element and attribute names.
Additionally, because of how easy it is to analyze XML trees, it is
easy to build numerous software engineering tools.  For example, it
was trivial to build a call-graph extractor (see
\figref{call-graph}).
...
ETL intentionally restricts the set of programming constructs i to
...
evolution. ETL is already used by dozens of production applications,
thus providing constructive evidence that the set of available
constructs is sufficient for real-world use.
...
an XML document with root element \smtexttt{bl:template}\footnote{For
brevity, we use namespace prefixes consistently to refer to elements
in distinguished namespace URIs.} The template contains other elements
...
processing starts (see \ssecref{selection}).  The primary goal of
...
Requests made of the ETL server are always have an associated abstract
...
brand hierarchy may provide some basic functionality, while a
sub-brand may provide an overriding \smtexttt{header.htm}
implementation that specializes the banner.

\subsubsection{Conditionals and loops}

Both \smtexttt{bl:if} and
\smtexttt{bl:choose}/\B{}\smtexttt{bl:when}/\B{}\smtexttt{bl:otherwise}
constructs are supported and behave much as they do in XSLT\@.  The
primary difference is that ETL does not permit the generality of a
domain specific expression language, and instead uses pairs of slots
...
approximate the convenience of a \smtexttt{switch} statement:
...
markup-based.  It is this distinction that enables ETL to favor
...
fact that the second example contains several errors.
...
(and the contents of the \smtexttt{b} element are always executed).
...
the diverse needs of the template author. By allowing careful control
over which whitespace is deemed meaningful, we improve performance
and reduce memory and bandwidth consumption.
...
illustrated in \figref{ws-rules}: \smtexttt{@bl:space-nodes} controls
...
source template thus reducing that template to contain only the core
...
100\% performance improvement over the interpreted language ETL
...
cached URL-to-template mapping or resets that cache.  These special
...
\smtexttt{etl-server-conf.etl} template prior to handling HTTP
requests.
...
the URL and toolbar; 4) the warning and trace message console; and 5)
...
templates and the clusters of ETL Servers serve tens of millions of
...
Earlier work, including JavaML, argues that XML proves very useful as
...
templating language---an approach that proves valuable because
...
weaknesses for common usage: 1) its declarative model confuses
...
processing occurs.  XExpr~\cite{XEpr} and XInclude~\cite{XInclude} are
...
LAML~\cite{Laml,Laml-WWW2002} has similar goals to ETL, but takes the
opposite approach: LAML takes the simpler markup languages and embeds
them in Scheme, an especially powerful and expressive language.  Each
...
extra power of Scheme is in direct contradiction of our approach of
...
general purpose programming language.  That system is more aggressive
...
The Extensible Templating Language was born out of questioning the
...
especially easy to build.  In particular, we show how the structure of
...
experience running ETLS in production and it serves tens of millions
of requests per day.
...
pair of attributes must be given.  Second, we have found limitations
in using XSLT for source to source transformations of ETL templates.
An template developer may care about non-infoset syntactic artifacts
such as newlines between attributes, but XSLT discards those
subtleties.~\cite{VanDeVanter2001}
...
support form integrity, these features could defeat many
