...
was a very simple matter to build a call-graph extractor (see
\figref{call-graph}).
...
Language is an integral part of InfoSpace's ETL Server 2.0.  ETLS is
architected as an ISAPI extension to Microsoft's IIS and could be
easily ported to any web server that exposes a similar Extension
Control Block interface.  IIS is used to manage the raw socket
connections and many HTTP protocol details, but ETLS generates the
response for all incoming requests.  The implementation of ETLS is
about sixty thousand non-comment, non-blank lines of C++ code.
